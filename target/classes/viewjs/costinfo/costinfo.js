var urlarray = {
	InfoUrl : "/Reception/daily/info.html",
	removeUrl : "/Reception/daily/remove.html",
	addUrl : "/Reception/daily/add.html",
	updateUrl : "/Reception/daily/update.html"
};
$(function() {
	getPageInfo();
...
function tdInfo(i, item) {
	...
	html += '</td><td>' + datanull(item.payname) + '</td><td>'
			+ datanull(item.paymoney);
	html += '</td><td>' + datanull(item.createtime) + '</td><td>'
			+ datanull(item.user) + "</td><td>"
			+ incomeOrexpend(item.income_expend);
	html += "</td><td>" + datanull(item.remark);
...
function updatedlg(num, obj, flag) {
	...
	inputcontent.each(function(i, item) {
		if (i == 0) {
			...
			if(i==5){
				$(item).val(objarray[num].income_expend);
			}else{
