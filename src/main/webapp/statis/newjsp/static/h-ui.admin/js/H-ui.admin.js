...
function removeIframe(flag) {
	...
	//是否有数据更新,0 有
	if (0 == flag) {
		var url = getUrl(iframe.eq(i - 1)[0].innerHTML);
		iframe.eq(i - 1).show(0, iFrameRefresh(url));
	} else {
		iframe.eq(i - 1).show();
		iframe.eq(i).remove();
	}
...
function getUrl(str) {
	var begin = str.lastIndexOf("src=\"");
	var end = str.lastIndexOf("\"");
	str = str.substring(begin + 5, end);
	return str;
}
