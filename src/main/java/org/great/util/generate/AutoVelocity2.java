...
public class AutoVelocity2 {
	...
	public AutoVelocity2() {
		...
		templatelist.add(ve.getTemplate("template2\\bean.vm"));
		templatelist.add(ve.getTemplate("template2\\service.vm"));
		templatelist.add(ve.getTemplate("template2\\controller.vm"));
		templatelist.add(ve.getTemplate("template2\\Idao.vm"));
		templatelist.add(ve.getTemplate("template2\\bean.sql.vm"));
		// 临时文件生成的目标路径,压缩完成后删除
		list.add("src\\test\\replaceflage.java");
		list.add("src\\test\\replaceflageService.java");
		list.add("src\\test\\replaceflageController.java");
		list.add("src\\test\\IreplaceflageDao.java");
		list.add("src\\test\\replaceflage.sql");
	}
	...
	public String autocode(List<ColumnEntity> resultlist, String tbname) {
		...
		for (int i = 0; i < templatelist.size(); i++) {
			...
			} else if (list.get(i).lastIndexOf(".sql") != -1) {
				name = name.replace("replaceflage", str);
			} else {
